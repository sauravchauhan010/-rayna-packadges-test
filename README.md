...
Premium group departures portal for Rayna Tours — a customer-facing site with
package browsing, a hotel directory, agent (email-link) login for private
files, and an admin dashboard for managing departures and the agent access
list.
...
│   ├── agent-auth.js       Customer-facing agent login (Firebase Email Link sign-in)
...
```
config.js → firebase.js → views.js → render.js → {db-packages, db-agents, drive, admin-auth}.js → {db.js, dispatch.js, agent-auth.js} → main.js
```
...
- Firestore holds two collections under `artifacts/{APP_ID}/public/data/`:
  `packages` and `agents`.
- Authentication → Sign-in method → **Email link (passwordless sign-in)**
  must be enabled for agent login to work.
- Authentication → Settings → **Authorized domains** must include your
  deployed Vercel domain (and `localhost` for local testing).
