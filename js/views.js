...
import { state, esc, pkgJson } from './state.js';
      ...
      <section style="padding:20px 20px 20px;background:#fff;border-bottom:1px solid #ede9e1; width: 100%;">
        <div style="width:100%; padding: 0 10px;">
          <div style="display:flex;flex-direction:column;align-items:flex-start;gap:10px;">
            ...
            <div id="destination-pills" style="display:flex;gap:8px;flex-wrap:wrap;width:100%;justify-content:flex-start;">
  ...
  export function renderHotels() {
    ...
    if (state.agentUnlocked) {
      const isFolderEmpty = !state.folderLoading && !state.folderError && state.folderSheets.length === 0;
      const showToolbar = !state.folderLoading && !state.folderError && !isFolderEmpty;

      const breadcrumbBar = `
        <div style="padding:10px 18px;background:#faf8f4;border-bottom:1px solid #ede9e1;display:flex;align-items:center;gap:6px;flex-wrap:wrap;">
          ${state.folderStack.length > 1 ? `
            <button onclick="navigateFolderUp()" title="Back" style="width:24px;height:24px;flex-shrink:0;border-radius:6px;border:1px solid #ede9e1;background:#fff;cursor:pointer;font-size:13px;display:flex;align-items:center;justify-content:center;color:var(--navy);margin-right:2px;">←</button>
          ` : ''}
          ${state.folderStack.map((crumb, i) => {
            const isLast = i === state.folderStack.length - 1;
            return (i > 0 ? '<span style="color:#ccc;font-size:11px;">/</span>' : '')
              + (isLast
                ? '<span style="font-size:11px;font-weight:700;color:var(--navy);padding:2px 4px;">' + esc(crumb.name) + '</span>'
                : '<button onclick="navigateFolderCrumb(' + i + ')" style="font-size:11px;font-weight:600;color:var(--gold);background:none;border:none;cursor:pointer;padding:2px 4px;">' + esc(crumb.name) + '</button>');
          }).join('')}
        </div>
      `;
      ...
      const fileBody = state.folderLoading ? `
      ...
      ` : isFolderEmpty ? `
        <p style="text-align:center;padding:40px 20px;font-size:14px;color:#888;margin:0;background:#fff;">This folder is empty.</p>
      ` : `
        ${state.folderSheets.filter(f => !state.folderSearch || f.name.toLowerCase().includes(state.folderSearch.toLowerCase())).map((file, i) => {
          ...
          return '<div onclick="openPromoFile(\'' + file.id + '\')" class="file-row" style="display:flex;align-items:center;gap:14px;padding:13px 18px;border-bottom:1px solid #f0ece4;cursor:pointer;background:#fff;' + bg + '">'
            ...
            + '</div>'
            + '<span style="font-size:10px;font-weight:600;color:#aaa;letter-spacing:0.05em;text-transform:uppercase;flex-shrink:0;">' + (isFolder ? 'Open →' : 'Preview →') + '</span>'
            + '</div>';
        }).join('')}
      `;
      ...
      <main class="${state.__animateViewFade ? 'view-fade' : ''}" style="flex:1;width:100%;padding:0 20px 80px;">
        <!--
          ...
          --rayna-header-h, kept in sync by main.js) so it stays visible
        ...
        <div style="background:#fff;border-radius:0 0 8px 8px;border:1px solid #ede9e1;border-top:none;overflow:hidden;margin-top:-1px;">
          ${fileBody}
        </div>
        ${sheetTable}
      </main>`;
      ...
      <div class="flex flex-col lg:flex-row gap-6 items-start">
        ...
        <div class="lg:sticky lg:top-[74px] w-full lg:max-w-[320px] flex-shrink-0" style="background:#fff;border-radius:8px;border:1px solid #ede9e1;padding:18px;">
          ...
          <form onsubmit="handleAgentSubmit(event)" style="display:flex;flex-direction:column;gap:12px;">
            ...
            <div>
              <label class="field-label" for="f-agt-password">Password ${state.editingAgentId ? '' : '*'}</label>
              <input class="field" id="f-agt-password" type="text" placeholder="${state.editingAgentId ? 'Leave blank to keep current password' : 'Set a password for this agent'}" value="${esc(state.agentDraft.password)}" ${state.editingAgentId ? '' : 'required'} />
            </div>
            <div style="display:flex;flex-direction:column;gap:6px;padding-top:2px;">
              <button type="submit" class="btn-gold" style="width:100%;justify-content:center;padding:9px;border-radius:4px;">
                  ...
                  <tbody>
                    ${filtered.map((agent, index) => `
                      <tr class="${state.editingAgentId === agent.id ? 'editing-row' : ''}">
                        ...
                        <td style="font-size:12px;color:#555;font-family:monospace;letter-spacing:0.03em;">${agent.hasPassword ? '••••••••' : '<span style=\"color:#c33;\">not set</span>'}</td>
                        <td style="text-align:right;padding:10px 12px;">
                          <div style="display:flex;justify-content:flex-end;gap:4px;">
