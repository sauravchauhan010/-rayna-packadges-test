...
function packDestinationPills() {
  ...
  export function render() {
    ...
    // Only replay the view fade-in / card stagger animations when the view
    // actually changed (e.g. switching Packages <-> Hotels, or the very
    // first paint) or when packages just finished loading for the first
    // time. Every other render (typing in search, background Firestore
    // updates while already looking at the page) skips them — otherwise
    // the whole page would flicker/blink on every keystroke or data update.
    const viewKey = state.view + ':' + state.activeView;
    state.__animateViewFade = state.__lastViewKey !== viewKey;
    state.__lastViewKey = viewKey;

    state.__animateCards = state.__animateViewFade || (!state.isDbLoading && !state.__packagesRevealed);
    if (!state.isDbLoading) state.__packagesRevealed = true;
    ...
    // sub-header below it (e.g. the agent Private Files toolbar) sits
    // right underneath instead of overlapping or leaving a gap.
